...
- Public Deployment (Vercel): `TODO_ADD_VERCEL_URL`
- Backend Feed:
...
### 4. Bottlenecks Identified / 已识别瓶颈

1. **Main-thread render pressure**
...
3. **Snapshot resync burst cost**
   - 发生 gap 后重新拉快照会短时间增加网络与处理压力。
   - 缓解：事件缓冲 + 快照回放，尽快恢复一致性。

### 5. Scaling Strategy (10x) / 10倍负载扩展策略
